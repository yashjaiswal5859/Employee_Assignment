...
def init_db_connections(app):
    ...
    MasterSession = sessionmaker(
        bind=master_engine, autoflush=False, expire_on_commit=False
    )
...
def get_master_session():
    if MasterSession is None:
        raise RuntimeError(
            "Master DB session is not initialized. Call init_db_connections(app) first."
        )
...
def get_replica_session():
    if ReplicaSession is None:
        raise RuntimeError(
            "Replica DB session is not initialized. Call init_db_connections(app) first."
        )
