...
def _test_create_employee_invalid_name(client):
    ...
    passed = (
        ...
        and "Validation Error" in response.get_json().get("error", "")
    )
...
def _test_create_employee_duplicate_email(client):
    ...
    passed = (
        ...
        and "already exists" in response.get_json().get("error", "")
    )
...
def _test_get_all_employees(client):
    ...
    actual = response.get_json()
    passed = (
        response.status_code == 200 and isinstance(actual, list) and len(actual) == 6
    )
    return _status(
        f"GET ALL - returns list (expected 6, got {len(actual) if isinstance(actual, list) else 0})",
        passed,
    )
...
def _test_get_employee_by_id_success(client):
    ...
    passed = (
        ...
        and actual.get("id") == alice_uuid
        and actual.get("name") == "Alice Johnson"
    )
    return _status(f"GET BY ID - valid id (id={alice_uuid})", passed)


def _test_get_employee_by_id_not_found(
    client, mock_id="99999999-9999-9999-9999-999999999999"
):
    response = client.get(f"/employees/{mock_id}")
    passed = (
        response.status_code == 404
        and f"Employee with ID {mock_id} not found"
        in response.get_json().get("error", "")
    )
    return _status(f"GET BY ID - not found (id={mock_id})", passed)


def _test_get_employee_by_id_invalid_format(client):
    response = client.get("/employees/1")
    passed = (
        response.status_code == 400
        and "Invalid employee ID format" in response.get_json().get("error", "")
    )
    return _status("GET BY ID - reject invalid UUID format", passed)


def _test_update_employee_success(client):
    alice_uuid = "11111111-1111-1111-1111-111111111111"
    mock_data = {"name": "Alice Updated", "department": "Management"}
    response = client.put(f"/employees/{alice_uuid}", json=mock_data)
    time.sleep(1)
    actual = response.get_json()
    passed = (
        response.status_code == 200
        and actual.get("name") == "Alice Updated"
        and actual.get("department") == "Management"
    )
...
def _test_update_employee_duplicate_email(client):
    ...
    passed = (
        response.status_code == 409
        and "is already in use" in response.get_json().get("error", "")
    )
...
def _test_delete_employee_success(client):
    ...
    passed = (
        response.status_code == 200
        and f"Employee with ID {alice_uuid} deleted successfully"
...
def run_tests():
    ...
    if seed_master:
        print(
            f"  {GREEN}[OK]{RESET} Seed data replicated to TEST REPLICA automatically"
        )
        time.sleep(1)
    else:
    ...
    with app.app_context():
        ...
        all_results.append(
            _test_get_employee_by_id_not_found(
                client, "99999999-9999-9999-9999-999999999999"
            )
        )
        all_results.append(_test_get_employee_by_id_invalid_format(client))
